...
class ModelMonitor:
    ...
    def analyze_drift(self, prediction_data):
        ...
        if self.reference_data is None or len(prediction_data) == 0:
            logger.warning("Cannot analyze drift: missing reference data or predictions")
            return None
        ...
        logger.info(f"Drift analysis: score={overall_drift:.4f}, drift_detected={result['drift_detected']}")
    ...
    def _generate_visualizations(self, report_dir, drift_analysis):
        ...
        if drift_analysis.get("feature_drift"):
            ...
            drift_values = [drift_analysis["feature_drift"][f]["normalized_difference"] for f in features]
            ...
            # Plot
            plt.barh(features, drift_values, color=plt.cm.viridis(np.array(drift_values) / max(max(drift_values), 1)))
        ...
        if len(self.drift_history) > 1:
            ...
            timestamps = [datetime.fromisoformat(d["timestamp"]) for d in self.drift_history]
    ...
    def run_monitoring_cycle(self):
        ...
        if health_response.status_code != 200:
            logger.error(f"Model API health check failed: {health_response.status_code}")
            return False
