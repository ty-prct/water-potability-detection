...
try:
    from scripts.deploy_api import app, ab_testing_config
    client = TestClient(app)
except ImportError:
    # For CI environment where model might not be available
    pytestmark = pytest.mark.skip("API module could not be loaded, skipping tests")
...
@pytest.mark.skipif(not os.path.exists("results/best_model.pkl") or 
                    not os.path.exists("models/random_forest_model.pkl"), 
...
def test_ab_testing_configuration():
    ...
    response = client.post("/api/ab-testing/configure", 
                           json={"traffic_split": {"A": 0.75, "B": 0.25}})
    assert response.status_code == 200
...
def test_prediction_with_ab_testing():
    ...
    assert ab_stats["metrics"]["A"]["requests"] > 0 or ab_stats["metrics"]["B"]["requests"] > 0
